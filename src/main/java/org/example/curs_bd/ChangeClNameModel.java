...
public class ChangeClNameModel {
    ...
    public static void changeName(ActionEvent event, String clName) {
        ...
        try {
            ...
            psCheckUserExists.setString(1, clName);
            resultSet = psCheckUserExists.executeQuery();

                psInsert = connection.prepareStatement("UPDATE owners SET name = ? WHERE owner_id = ?");
                psInsert.setString(1, clName);
                psInsert.setInt(2, Singleton.getInstance().getId());
                psInsert.executeUpdate();// Execute the insert statement
                Singleton.getInstance().setName(clName);
                switchMain(event, "loggedCl.fxml", clName);
